...
class HttpClient {
  static async get<T>({ url, method, body }: HttpClientGet): Promise<T> {
    return new Promise((resolve, reject) => {
      let request = new XMLHttpRequest()
      request.open(method, url, true)
...
let apiKey = '2d0da94d2ab0d469ceb6e1d7ef28802d'
let requestToken: string
let username: string = 'gabrielribeirof'
let password: string = 'passthe7856'
...
async function searchMovies(query: string) {
  ...
  const { results } = await HttpClient.get<MovieSearchResponse>({
...
async function createRequestToken() {
  ...
  let result = await HttpClient.get<CreateRequestTokenResponse>({
    url: `https://api.themoviedb.org/3/authentication/token/new?api_key=${apiKey}`,
    method: "GET"
  })
...
async function createSession() {
  ...
  let result = await HttpClient.get<CreateSessionResponse>({
    url: `https://api.themoviedb.org/3/authentication/session/new?api_key=${apiKey}&request_token=${requestToken}`,
    method: "GET"
  })
